...
function overviewHtml(data) {
  ...
  return `<section class="room-overview room-overview-v64">
    ...
    <div class="panel"><div class="panel-head"><div><div class="panel-title">Status Kelas</div><p class="panel-copy">Status keanggotaan dan akses kelas saat ini.</p></div></div><div class="detail-list">
      ${detail('Role kamu',roleLabel(c.role||'MEMBER'))}${detail('Visibilitas',c.visibility||'-')}${detail('Anggota aktif',String((data.members||[]).length))}${detail('Class Code',c.class_code||'-')}${detail('Link Kelas',publicClassLinksUrl(c.class_code||'',c.public_slug||''))}
    </div></div>
...
function scheduleRoom(items,p,attendanceItems) {
  ...
  const week=sorted.filter(x=>dateMs(x.start_at)>=now&&dateMs(x.start_at)<=weekEnd&&String(x.schedule_state||'NORMAL')!=='CANCELLED').length;
  ...
  sorted.forEach(x=>{const key=x.recurrence_group_id||`TITLE:${String(x.title||'').toLowerCase()}`;if(!groups.has(key))groups.set(key,{title:x.title||'Jadwal',items:[]});groups.get(key).items.push(x);});
  const dateStrip=sorted.filter(x=>dateMs(x.start_at)>=now).slice(0,10).map(x=>`<button type="button" class="schedule-date-chip state-${String(x.schedule_state||'NORMAL').toLowerCase()}" data-schedule-detail="${esc(x.schedule_id)}"><strong>${esc(dayPart(x.start_at))}</strong><span>${esc(monthPart(x.start_at))}</span></button>`).join('');
  const groupHtml=[...groups.values()].map(group=>{
    ...
    return `<section class="schedule-series-card"><div class="schedule-series-head"><div><span class="eyebrow">MATA KULIAH / KEGIATAN</span><h3>${esc(group.title)}</h3><small>${sessions.length} sesi · ${normal} normal${rev?` · ${rev} revisi`:''}${bat?` · ${bat} batal`:''}</small></div><span class="material-symbols-rounded schedule-series-icon">event_note</span></div><div class="schedule-session-list">${sessions.map(x=>{const at=attendanceBySchedule[String(x.schedule_id)]||null;const session=x._sessionNo?`Sesi ${String(x._sessionNo).padStart(2,'0')}`:'Sesi';const locationLabel=isWebUrl(x.location)?'Online • Zoom/Meet':(x.location||'Tanpa lokasi');return `<article class="schedule-session-row state-${String(x.schedule_state||'normal').toLowerCase()}"><button type="button" class="schedule-session-main" data-schedule-detail="${esc(x.schedule_id)}"><span class="academic-date-tile compact"><strong>${esc(dayPart(x.start_at))}</strong><span>${esc(monthPart(x.start_at))}</span></span><span class="schedule-session-copy"><span><b>${esc(session)}</b><i>${esc(timeRange(x.start_at,x.end_at))}</i></span><small>${esc(locationLabel)}${x.change_note?` · ${esc(excerpt(x.change_note,80))}`:''}</small></span>${scheduleStateBadge(x)}${x.recurrence_group_id?'<span class="material-symbols-rounded recurring-icon" title="Berulang">repeat</span>':''}<span class="material-symbols-rounded row-chevron">chevron_right</span></button><div class="academic-row-actions compact-actions">${at?`<button type="button" class="icon-btn mini" data-copy-attendance="${esc(at.public_token||'')}" title="Salin link absensi">${svg('i-link')}</button>`:''}${(p.can_manage_schedule||p.can_manage_academic)?`<button type="button" class="icon-btn mini" data-edit-schedule="${esc(x.schedule_id)}" title="Edit sesi"><span class="material-symbols-rounded">edit_calendar</span></button>`:''}</div></article>`;}).join('')}</div></section>`;
  }).join('');
  return `<section class="class-academic-panel schedule-room-v656"><div class="academic-room-toolbar panel"><div><div class="panel-title">Jadwal Kelas</div><p class="panel-copy">Dikelompokkan per mata kuliah/kegiatan. Sesi tetap terpisah untuk revisi, pembatalan, dan absensi.</p></div>${(p.can_manage_schedule||p.can_manage_academic)?`<button id="create-schedule" class="btn btn-primary compact-create-btn">${svg('i-plus')} Tambah</button>`:''}</div>
    ...
    ${dateStrip?`<div class="schedule-date-strip">${dateStrip}</div><div class="schedule-color-note"><span><i class="dot normal"></i>Normal</span><span><i class="dot changed"></i>Revisi</span><span><i class="dot cancelled"></i>Batal</span></div>`:''}
...
async function uploadAcademicFiles(files,module,statusEl){
  ...
  for(let i=0;i<list.length;i++){
    ...
    else { if(file.size>1050000)throw new Error(`${file.name} terlalu besar. Maksimal sekitar 1 MB.`); dataUrl=await fileToDataUrl(file); }
...
function attachmentPickerHtml(id,label='Lampiran'){
  return `<div class="field"><label>${esc(label)}</label><input id="${id}" type="file" multiple class="control" accept="image/*,application/pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt"><div class="form-help">Maks. 12 file. Gambar dikompresi; dokumen maksimal sekitar 1 MB/file. Tidak ada preview otomatis.</div><div class="compact-upload-note" data-upload-note-for="${id}">Belum ada file dipilih.</div></div>`;
}
function bindAttachmentPicker(id){const input=document.getElementById(id),note=document.querySelector(`[data-upload-note-for="${id}"]`);if(!input||!note)return;input.onchange=()=>{const files=[...input.files].slice(0,12);note.innerHTML=files.length?files.map(f=>`<span class="upload-file-chip"><span class="material-symbols-rounded">attach_file</span>${esc(f.name)} <small>${esc(formatBytes(f.size))}</small></span>`).join(''):'Belum ada file dipilih.';};}

function openCreateAnnouncement(){
  showModal(`<div class="modal-head"><div><div class="eyebrow">Pengumuman</div><h2>Buat Pengumuman</h2></div><button class="icon-btn mini" data-close-modal>${svg('i-close')}</button></div><form id="announcement-create-form"><div class="form-grid"><div class="field"><label>Judul</label><input name="title" class="control" required minlength="3"></div><div class="field"><label>Prioritas</label><select name="priority" class="control"><option value="NORMAL">Normal</option><option value="HIGH">High</option><option value="URGENT">Urgent</option><option value="LOW">Low</option></select></div></div><div class="field"><label>Isi</label><textarea name="body" class="control" rows="4" required></textarea></div><div class="field"><label>Link / video</label><input name="reference_url" type="url" class="control" placeholder="https://youtube.com/... atau tautan referensi"></div>${attachmentPickerHtml('announcement-files','Gambar / PDF / File')} ${formFooter('Publikasikan')}</form>`);
  bindAttachmentPicker('announcement-files');
  const form=document.getElementById('announcement-create-form');form.onsubmit=async e=>{e.preventDefault();const btn=document.getElementById('academic-form-submit'),status=document.getElementById('academic-form-status'),old=btn.innerHTML;if(btn.disabled)return;btn.disabled=true;btn.innerHTML='<span class="btn-spinner"></span><span>Menyimpan…</span>';try{const files=[...document.getElementById('announcement-files').files];const fileIds=await uploadAcademicFiles(files,'ANNOUNCEMENT',status);const payload=Object.fromEntries(new FormData(form));payload.class_id=state.selectedClassId;payload.attachment_file_ids=fileIds;await api('createAnnouncement',payload);closeModal();toast('Pengumuman dipublikasikan.');await refreshAcademicAfterMutation();}catch(err){status.className='request-status error';status.textContent=err.message;}finally{btn.disabled=false;btn.innerHTML=old;}};
}
...
function renderMaterialUploadList(files,statuses={}){
  ...
  slot.innerHTML=files.length?files.map((f,i)=>{const st=statuses[i]||'WAIT';return `<div class="material-upload-item status-${st.toLowerCase()}"><span class="material-symbols-rounded">${st==='DONE'?'check_circle':st==='UPLOADING'?'sync':'attach_file'}</span><div><strong>${esc(f.name)}</strong><small>${st==='DONE'?'Terupload ✓':st==='UPLOADING'?'Mengupload…':'Menunggu upload'}</small></div></div>`;}).join(''):'<span class="form-help">Belum ada file dipilih.</span>';
}
async function bindMaterialCreateForm(){
  const form=document.getElementById('material-create-form');form.onsubmit=async e=>{e.preventDefault();const btn=document.getElementById('academic-form-submit'),status=document.getElementById('academic-form-status'),files=[...document.getElementById('material-files').files].slice(0,12),statuses={};if(btn.disabled)return;btn.disabled=true;btn.innerHTML='<span class="btn-spinner"></span><span>Mengupload…</span>';status.className='request-status progress';status.textContent='Menyiapkan lampiran…';const fileIds=[];try{for(let i=0;i<files.length;i++){statuses[i]='UPLOADING';renderMaterialUploadList(files,statuses);const f=files[i];let dataUrl='',filename=f.name;if(String(f.type||'').startsWith('image/')){const processed=await compressImageFile(f,{maxEdge:1600,targetBytes:700000});dataUrl=processed.dataUrl;filename=processed.name;}else{if(f.size>1050000)throw new Error(`${f.name} terlalu besar. Maksimal sekitar 1 MB.`);dataUrl=await fileToDataUrl(f);}const up=await api('uploadAcademicAttachment',{class_id:state.selectedClassId,module:'MATERIAL',filename,data_url:dataUrl});if(up?.file?.file_id)fileIds.push(up.file.file_id);statuses[i]='DONE';renderMaterialUploadList(files,statuses);}status.textContent='Lampiran selesai. Menyimpan materi…';const payload=Object.fromEntries(new FormData(form));payload.class_id=state.selectedClassId;payload.attachment_file_ids=fileIds;payload.reference_url=payload.url||'';const result=await api('createMaterial',payload);closeModal();toast(`Materi tersimpan${fileIds.length?` dengan ${fileIds.length} lampiran`:''}.`);await refreshAcademicAfterMutation();return result;}catch(err){status.className='request-status error';status.textContent=err.message;}finally{btn.disabled=false;btn.innerHTML='Terbitkan Materi';}};
}
...
function buildActivitySummary(records){
  const groups={};
  (records||[]).forEach(r=>{const name=String(r.activity_name||r.session_title||'Kegiatan').trim()||'Kegiatan';const key=name.toLowerCase();if(!groups[key])groups[key]={name,sessionIds:new Set(),PRESENT:0,SICK:0,PERMIT:0,ABSENT:0,UNMARKED:0,ZOOM:0,YOUTUBE:0,OFFLINE:0,OTHER:0,ON_TIME:0,LATE:0,total:0};const g=groups[key];g.sessionIds.add(String(r.attendance_id||''));const st=String(r.attendance_status||'UNMARKED').toUpperCase();g[st]=(g[st]||0)+1;const ch=String(r.attendance_channel||'').toUpperCase();if(ch)g[ch]=(g[ch]||0)+1;const pn=String(r.punctuality||'').toUpperCase();if(pn)g[pn]=(g[pn]||0)+1;g.total+=1;});
...
function analyticsSheetRows(title,records,members){
  ...
  const rows=[[xlsxCell(title,2)],[''],[xlsxCell('RINGKASAN CEPAT',1)],['Jumlah sesi',sessions.length],['Jumlah mahasiswa',memberSummary.length],['Total record',records.length],['Hadir',present],['Sakit',sick],['Izin',permit],['Alpa',absent],['Belum',unmarked],['% Presensi',rate+'%'],['Zoom',records.filter(r=>r.attendance_channel==='ZOOM').length],['YouTube',records.filter(r=>r.attendance_channel==='YOUTUBE').length],['Offline',records.filter(r=>r.attendance_channel==='OFFLINE').length],['Tepat waktu',records.filter(r=>r.punctuality==='ON_TIME').length],['Terlambat',records.filter(r=>r.punctuality==='LATE').length],[],[xlsxCell('REKAP MAHASISWA',1)],[xlsxCell('Nama',1),xlsxCell('Username',1),xlsxCell('Hadir',1),xlsxCell('Sakit',1),xlsxCell('Izin',1),xlsxCell('Alpa',1),xlsxCell('Belum',1),xlsxCell('Zoom',1),xlsxCell('YouTube',1),xlsxCell('Tepat Waktu',1),xlsxCell('Terlambat',1),xlsxCell('% Presensi',1)]];
  memberSummary.forEach(m=>rows.push([m.full_name||'',m.username||'',m.PRESENT||0,m.SICK||0,m.PERMIT||0,m.ABSENT||0,m.UNMARKED||0,m.ZOOM||0,m.YOUTUBE||0,m.ON_TIME||0,m.LATE||0,(m.attendance_rate||0)+'%']));
  rows.push([], [xlsxCell('REKAP PER SESI',1)], [xlsxCell('Sesi',1),xlsxCell('Tanggal',1),xlsxCell('Hadir',1),xlsxCell('Sakit',1),xlsxCell('Izin',1),xlsxCell('Alpa',1),xlsxCell('Belum',1),xlsxCell('Zoom',1),xlsxCell('YouTube',1),xlsxCell('Tepat Waktu',1),xlsxCell('Terlambat',1),xlsxCell('% Presensi',1)]);
  sessions.forEach(x=>rows.push([x.session_no?`Sesi ${String(x.session_no).padStart(2,'0')}`:'Sesi',shortDateTime(x.start_at),x.PRESENT,x.SICK,x.PERMIT,x.ABSENT,x.UNMARKED,x.ZOOM||0,x.YOUTUBE||0,x.ON_TIME||0,x.LATE||0,x.attendance_rate+'%']));
  rows.push([], [xlsxCell('DETAIL PRESENSI',1)], [xlsxCell('Nama',1),xlsxCell('Username',1),xlsxCell('Sesi',1),xlsxCell('Tanggal',1),xlsxCell('Status',1),xlsxCell('Media',1),xlsxCell('Ketepatan',1),xlsxCell('Catatan',1)]);
  records.forEach(r=>rows.push([r.full_name||'',r.username||'',r.session_no?`Sesi ${String(r.session_no).padStart(2,'0')}`:'',shortDateTime(r.start_at),attendanceStatusLabel(r.attendance_status),attendanceChannelLabel(r.attendance_channel),punctualityLabel(r.punctuality),r.note||'']));
  return rows;
}
function exportAttendanceXlsx(data){
  ...
  const overview=[[xlsxCell('KELASKU — RINGKASAN PRESENSI',2)],['Scope',scope],['Jumlah sesi',data.session_count||0],['Jumlah mahasiswa',members.length],['Total record',records.length],['% Presensi',overallRate+'%'],[],[xlsxCell('STATUS KEHADIRAN',1)],['Hadir',pCount],['Sakit',sCount],['Izin',iCount],['Alpa',aCount],['Belum',uCount],[],[xlsxCell('MEDIA KEHADIRAN',1)],['Zoom',records.filter(r=>r.attendance_channel==='ZOOM').length],['YouTube',records.filter(r=>r.attendance_channel==='YOUTUBE').length],['Offline',records.filter(r=>r.attendance_channel==='OFFLINE').length],['Lainnya',records.filter(r=>r.attendance_channel==='OTHER').length],[],[xlsxCell('KETEPATAN WAKTU',1)],['Tepat waktu',records.filter(r=>r.punctuality==='ON_TIME').length],['Terlambat',records.filter(r=>r.punctuality==='LATE').length],['Tidak dinilai',records.filter(r=>!r.punctuality).length],[],[xlsxCell('RINGKASAN PER MATA KULIAH / KEGIATAN',1)],[xlsxCell('Kegiatan',1),xlsxCell('Sesi',1),xlsxCell('Record',1),xlsxCell('Hadir',1),xlsxCell('Sakit',1),xlsxCell('Izin',1),xlsxCell('Alpa',1),xlsxCell('Belum',1),xlsxCell('Zoom',1),xlsxCell('YouTube',1),xlsxCell('Offline',1),xlsxCell('Tepat',1),xlsxCell('Terlambat',1),xlsxCell('% Presensi',1)]];
  summaries.forEach(g=>overview.push([g.name,g.sessions,g.total,g.PRESENT,g.SICK,g.PERMIT,g.ABSENT,g.UNMARKED,g.ZOOM||0,g.YOUTUBE||0,g.OFFLINE||0,g.ON_TIME||0,g.LATE||0,g.attendance_rate+'%']));
  const detail=[[xlsxCell('KELASKU — ALL RECORD',2)],[xlsxCell('Nama',1),xlsxCell('Username',1),xlsxCell('Kegiatan / Mata Kuliah',1),xlsxCell('Sesi',1),xlsxCell('Tanggal',1),xlsxCell('Status',1),xlsxCell('Media',1),xlsxCell('Ketepatan',1),xlsxCell('Catatan',1)]];records.forEach(r=>detail.push([r.full_name||state.user?.full_name||'',r.username||state.user?.username||'',r.activity_name||r.session_title||'',r.session_no?`Sesi ${String(r.session_no).padStart(2,'0')}`:'',shortDateTime(r.start_at),attendanceStatusLabel(r.attendance_status),attendanceChannelLabel(r.attendance_channel),punctualityLabel(r.punctuality),r.note||'']));
  const sheets=[{name:'ANALISIS',rows:overview},{name:data.can_manage?'ALL':'SAYA',rows:detail}];summaries.forEach((summary,index)=>{const activityRecords=records.filter(r=>String(r.activity_name||r.session_title||'').trim().toLowerCase()===String(summary.name).trim().toLowerCase());sheets.push({name:`${String(index+1).padStart(2,'0')}-${summary.name}`,rows:analyticsSheetRows(summary.name,activityRecords,scopeMembers)});});downloadXlsx(`KelasKu-Analitik-${state.selectedClassId}.xlsx`,sheets);
}
...
function settingsHtml(data) {
  const c=data.class||{},s=data.settings||{},p=data.permissions||{},links=data.class_links||[],publicUrl=publicClassLinksUrl(c.class_code||'',c.public_slug||'');
