...
function scheduleRoom(items,p,attendanceItems) {
  ...
  const groupHtml=[...groups.values()].map((group,idx)=>{
    ...
    return `<section class="schedule-series-card ${idx===0?'open':''}" data-schedule-group>
      <button type="button" class="schedule-series-head" data-schedule-group-toggle aria-expanded="${idx===0?'true':'false'}" aria-controls="${groupId}">
      ...
      <div class="schedule-session-list" id="${groupId}" ${idx===0?'':'hidden'}>${sessions.map(x=>{const at=attendanceBySchedule[String(x.schedule_id)]||null;const session=x._sessionNo?`Sesi ${String(x._sessionNo).padStart(2,'0')}`:'Sesi';const locationLabel=isWebUrl(x.location)?'Online • Zoom/Meet':(x.location||'Tanpa lokasi');const cancelled=String(x.schedule_state||'').toUpperCase()==='CANCELLED';return `<article class="schedule-session-row state-${String(x.schedule_state||'normal').toLowerCase()}"><button type="button" class="schedule-session-main" data-schedule-detail="${esc(x.schedule_id)}"><span class="academic-date-tile compact"><strong>${esc(dayPart(x.start_at))}</strong><span>${esc(monthPart(x.start_at))}</span></span><span class="schedule-session-copy"><span><b>${esc(session)}</b><i>${esc(timeRange(x.start_at,x.end_at))}</i></span><small>${esc(locationLabel)}${x.change_note?` · ${esc(excerpt(x.change_note,80))}`:''}</small></span>${scheduleStateBadge(x)}${x.recurrence_group_id?'<span class="material-symbols-rounded recurring-icon" title="Jadwal berulang">repeat</span>':''}<span class="material-symbols-rounded row-chevron">chevron_right</span></button><div class="academic-row-actions compact-actions">${at?`<button type="button" class="session-action-btn" data-attendance-id="${esc(at.attendance_id)}" title="Buka presensi"><span class="material-symbols-rounded">how_to_reg</span><span>Presensi</span></button><button type="button" class="icon-btn mini" data-copy-attendance="${esc(at.public_token||'')}" title="Salin link presensi">${svg('i-copy')}</button>`:''}${(p.can_manage_schedule||p.can_manage_academic)&&!cancelled?`<button type="button" class="icon-btn mini" data-edit-schedule="${esc(x.schedule_id)}" title="Edit sesi"><span class="material-symbols-rounded">edit_calendar</span></button>`:''}</div></article>`;}).join('')}</div>
    </section>`;
  }).join('');
  return `<section class="class-academic-panel schedule-room-v661"><div class="academic-room-toolbar panel"><div><div class="panel-title">Jadwal Kelas</div><p class="panel-copy">Jadwal aktif dikelompokkan per mata kuliah. Sesi yang selesai otomatis tidak memenuhi daftar aktif.</p></div>${(p.can_manage_schedule||p.can_manage_academic)?`<button id="create-schedule" class="btn btn-primary compact-create-btn">${svg('i-plus')} Tambah</button>`:''}</div>
    ...
    ${dateStrip?`<div class="schedule-date-strip">${dateStrip}</div><div class="schedule-color-note"><span><i class="dot normal"></i>Normal</span><span><i class="dot changed"></i>Revisi</span><span><i class="dot cancelled"></i>Batal</span><small>Warna hanya indikator, bukan tombol.</small></div>`:''}
...
function openScheduleDetail(id,data){
  ...
  const attendanceActions=attendance?`<div class="schedule-detail-attendance-actions"><button type="button" id="schedule-open-attendance" class="btn btn-primary"><span class="material-symbols-rounded">how_to_reg</span> Buka Presensi</button>${attendance.public_token?`<button type="button" id="schedule-copy-attendance" class="btn btn-secondary">${svg('i-copy')} Salin Link</button>`:''}</div>`:'';
  showModal(`<div class="modal-head"><div><div class="eyebrow">JADWAL • ${esc(shortDateTime(x.start_at))}</div><h2>${esc(x.title)}</h2></div><button class="icon-btn mini" data-close-modal>${svg('i-close')}</button></div><div class="detail-badge-row">${sessionBadge}${scheduleStateBadge(x)}${x.recurrence_group_id?'<span class="soft-chip">Jadwal Berulang</span>':''}${String(x.attendance_mode)==='AUTO'?'<span class="soft-chip">Absensi Auto</span>':''}</div><div class="detail-list academic-detail-list">${detail('Mulai',shortDateTime(x.start_at))}${detail('Selesai',x.end_at?shortDateTime(x.end_at):'-')}${detail('Lokasi',locationText)}${x.change_note?detail('Catatan perubahan',x.change_note):''}</div>${attendanceActions}${meeting?`<button type="button" id="schedule-meeting-link" class="btn btn-secondary btn-block"><span class="material-symbols-rounded">videocam</span> Buka Zoom / Meet</button>`:''}${x.description?`<div class="academic-detail-body">${nl2br(x.description)}</div>`:''}${can&&String(x.schedule_state)!=='CANCELLED'?`<div class="modal-actions"><button type="button" id="detail-edit-schedule" class="btn btn-secondary"><span class="material-symbols-rounded">edit_calendar</span> Edit Sesi</button><button type="button" id="detail-cancel-schedule" class="btn btn-danger"><span class="material-symbols-rounded">event_busy</span> Batalkan</button></div>`:String(x.schedule_state)==='CANCELLED'?'<div class="alert info">Jadwal dibatalkan dan dikunci. Koreksi hanya melalui pengelola data.</div>':''}`);
...
function activitySummaryHtml(items){
  ...
  return `<div class="analytics-table-wrap"><table class="analytics-table analytics-summary-table"><thead><tr><th>Kegiatan / Mata Kuliah</th><th>Mahasiswa</th><th>Sesi</th><th>Record</th><th>Hadir</th><th>Sakit</th><th>Izin</th><th>Alpa</th><th>Belum</th><th>% Presensi</th></tr></thead><tbody>${items.map(x=>{const names=x.student_names||[];const shown=names.slice(0,2).join(', ')+(names.length>2?` +${names.length-2}`:'');return `<tr><td><strong>${esc(x.name)}</strong></td><td><span class="analytics-student-names" title="${esc(names.join(', '))}">${esc(shown||'-')}</span></td><td>${x.sessions}</td><td>${x.total}</td><td>${x.PRESENT}</td><td>${x.SICK}</td><td>${x.PERMIT}</td><td>${x.ABSENT}</td><td>${x.UNMARKED}</td><td><strong>${x.attendance_rate}%</strong></td></tr>`}).join('')}</tbody></table></div>`;
}
...
function analyticsSheetRows(title,records,members){
  ...
  const matrixSessions=sessions.slice(0,32);const statusCode=r=>{const st=String(r.attendance_status||'UNMARKED').toUpperCase();if(st==='PRESENT'){const ch=String(r.attendance_channel||'').toUpperCase();return ch==='ZOOM'?'H-Z':ch==='YOUTUBE'?'H-YT':ch==='OFFLINE'?'H-O':'H';}if(st==='SICK')return'S';if(st==='PERMIT')return'I';if(st==='ABSENT')return'A';return'-';};
  rows.push([], [xlsxCell('MATRIX PRESENSI PER SESI',1)], ['Kode: H-Z=Hadir Zoom | H-YT=Hadir YouTube | H-O=Hadir Offline | S=Sakit | I=Izin | A=Alpa | -=Belum'], [xlsxCell('Nama',1),xlsxCell('Username',1),...matrixSessions.map((x,i)=>xlsxCell(x.session_no?`S${String(x.session_no).padStart(2,'0')}`:`S${String(i+1).padStart(2,'0')}`,1)),xlsxCell('Hadir',1),xlsxCell('Sakit',1),xlsxCell('Izin',1),xlsxCell('Alpa',1),xlsxCell('% Presensi',1)]);
  memberSummary.forEach(m=>{const mine=records.filter(r=>String(r.user_id||'')===String(m.user_id||''));const cells=matrixSessions.map(sess=>{const rec=mine.find(r=>String(r.attendance_id||'')===String(sess.attendance_id||''));return rec?statusCode(rec):'-';});rows.push([m.full_name||'',m.username||'',...cells,m.PRESENT||0,m.SICK||0,m.PERMIT||0,m.ABSENT||0,(m.attendance_rate||0)+'%']);});
