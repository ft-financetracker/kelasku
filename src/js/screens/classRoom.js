...
function openAnnouncementDetail(id,data){
  ...
  showModal(`<div class="modal-head announcement-detail-head"><div><div class="eyebrow">PENGUMUMAN • ${esc(shortDateTime(x.published_at))}</div><div class="modal-title-badge"><h2>${esc(x.title)}</h2><span class="status-badge status-${String(x.priority||'normal').toLowerCase()}">${esc(x.priority||'NORMAL')}</span></div></div><button class="icon-btn mini" data-close-modal>${svg('i-close')}</button></div>${x.updated_at&&x.updated_at!==x.published_at?'<div class="detail-badge-row"><span class="soft-chip">Diperbarui</span></div>':''}<div class="academic-detail-body">${nl2br(x.body||'')}</div>${can?`<div class="modal-actions"><button type="button" id="detail-edit-announcement" class="btn btn-secondary"><span class="material-symbols-rounded">edit</span> Edit</button><button type="button" id="detail-delete-announcement" class="btn btn-danger">${svg('i-close')} Hapus</button></div>`:''}`);
